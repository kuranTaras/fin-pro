<section class="best">
    <div class="container">
        <div class="best-title">
            <?php the_sub_field('title'); ?>
        </div>
        <div class="rating__filters">
            <div class="rating__filters-wrapper">
                <?php
                // Основний запит до бази даних для отримання категорій типу "казино"
                $args = array(
                    'taxonomy' => 'bonus_category', // Подати 'category' для категорій, або використовуйте назву своєї таксономії, якщо ви створили власну
                    'object_type' => 'bonus', // Замініть 'casino' на назву вашого типу запису
                    'hide_empty' => false, // Виводимо порожні категорії
                );

                $categories = get_categories($args);

                // Перевірка, чи є категорії
                if ($categories) {
                    foreach ($categories as $category) {
                        ?>
                        <div class="rating__filter" data-sort="sort-<?php echo $category->slug ?>">
                            <?php echo $category->name ?>
                        </div>
                        <?php
                    }
                } else {
                    echo 'Категорії не знайдені.';
                }
                ?>
            </div>
            <div class="rating__filters-about">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M8 0C6.41775 0 4.87103 0.469192 3.55544 1.34824C2.23985 2.22729 1.21447 3.47672 0.608967 4.93853C0.00346628 6.40034 -0.15496 8.00887 0.153721 9.56072C0.462403 11.1126 1.22433 12.538 2.34315 13.6569C3.46197 14.7757 4.88743 15.5376 6.43928 15.8463C7.99113 16.155 9.59966 15.9965 11.0615 15.391C12.5233 14.7855 13.7727 13.7602 14.6518 12.4446C15.5308 11.129 16 9.58225 16 8C15.9978 5.87895 15.1542 3.84542 13.6544 2.34562C12.1546 0.845814 10.121 0.00223986 8 0ZM8 14.7692C6.66117 14.7692 5.35241 14.3722 4.23922 13.6284C3.12603 12.8846 2.2584 11.8274 1.74605 10.5905C1.2337 9.35356 1.09965 7.99249 1.36084 6.67939C1.62203 5.36629 2.26674 4.16012 3.21343 3.21343C4.16013 2.26674 5.36629 1.62203 6.67939 1.36084C7.99249 1.09965 9.35356 1.2337 10.5905 1.74605C11.8274 2.25839 12.8846 3.12602 13.6284 4.23922C14.3722 5.35241 14.7692 6.66117 14.7692 8C14.7672 9.79468 14.0534 11.5153 12.7843 12.7843C11.5153 14.0534 9.79469 14.7672 8 14.7692ZM9.23077 11.6923C9.23077 11.8555 9.16594 12.012 9.05053 12.1274C8.93512 12.2429 8.7786 12.3077 8.61539 12.3077C8.28897 12.3077 7.97591 12.178 7.7451 11.9472C7.51429 11.7164 7.38462 11.4033 7.38462 11.0769V8C7.22141 8 7.06488 7.93516 6.94947 7.81976C6.83407 7.70435 6.76923 7.54782 6.76923 7.38461C6.76923 7.2214 6.83407 7.06488 6.94947 6.94947C7.06488 6.83406 7.22141 6.76923 7.38462 6.76923C7.71104 6.76923 8.02409 6.8989 8.2549 7.12971C8.48572 7.36053 8.61539 7.67358 8.61539 8V11.0769C8.7786 11.0769 8.93512 11.1418 9.05053 11.2572C9.16594 11.3726 9.23077 11.5291 9.23077 11.6923ZM6.76923 4.61538C6.76923 4.43282 6.82337 4.25435 6.9248 4.10255C7.02623 3.95075 7.17039 3.83244 7.33906 3.76257C7.50773 3.69271 7.69333 3.67443 7.87239 3.71004C8.05145 3.74566 8.21593 3.83358 8.34502 3.96267C8.47412 4.09176 8.56203 4.25624 8.59765 4.4353C8.63327 4.61436 8.61499 4.79996 8.54512 4.96863C8.47526 5.1373 8.35694 5.28147 8.20514 5.38289C8.05334 5.48432 7.87488 5.53846 7.69231 5.53846C7.44749 5.53846 7.21271 5.44121 7.0396 5.2681C6.86649 5.09499 6.76923 4.8602 6.76923 4.61538Z" fill="#00D1FF"/>
                </svg>
                <span>
                    <?php the_sub_field('about_rating'); ?>
                </span>
            </div>
        </div>
        <div class="best__tabs">
            <?php
            // Основний запит до бази даних для отримання категорій типу "казино"
            $args = array(
                'taxonomy' => 'bonus_category', // Подати 'category' для категорій, або використовуйте назву своєї таксономії, якщо ви створили власну
                'object_type' => 'bonus', // Замініть 'casino' на назву вашого типу запису
                'hide_empty' => false, // Виводимо порожні категорії
            );

            $categories = get_categories($args);

            // Перевірка, чи є категорії
            if ($categories) {
                foreach ($categories as $index => $category) {
                    $active = '';
                    if ($index == 0) {
                        $active = 'active';
                    }
                    ?>
                    <div class="best__tab <?php echo $active ?>">
                        <div class="best__slider swiper">
                            <div class="best__wrapper swiper-wrapper">
                                <?php
                                // Визначаємо новий запит на пости типу "Казино"


                                $args = array(
                                    'post_type' => 'bonus',
                                    'post_status' => 'publish',
                                    'posts_per_page' => -1, // -1 означає показати всі пости
                                    'tax_query' => array(
                                        array(
                                            'taxonomy' => 'bonus_category', // Таксономія, до якої належить категорія бонусу
                                            'field' => 'slug',
                                            'terms' => $category->slug, // Слаг (slug) категорії "best"
                                        ),
                                    ),
                                );

                                // Виконуємо запит
                                $casino_query = new WP_Query( $args );

                                // Виводимо пости типу "Казино"
                                if ( $casino_query->have_posts() ) :
                                    while ( $casino_query->have_posts() ) : $casino_query->the_post();

                                        $categories = get_the_terms(get_the_ID(), 'bonus_category');
                                        $category_slugs = array();

                                        if ($categories) {
                                            foreach ($categories as $category) {
                                                $category_slugs[] = $category->slug;
                                            }
                                        }
                                        ?>
                                        <div class="best__slide swiper-slide <?php
                                        foreach ($category_slugs as $sort) {
                                            echo 'sort-' .$sort . ' ';
                                            if ($sort == 'best') {
                                                echo ' active ';
                                            }
                                        }
                                        ?>">
                                            <div class="best__slide-wrapper">
                                                <div class="best__slide-img">
                                                    <img src="<?php the_field('background_image'); ?>" alt="">
                                                    <img src="<?php the_field('logo'); ?>" alt="">
                                                </div>
                                                <div class="best__slide-text">
                                                    <div class="best__slide-title">
                                                        <?php the_field('title'); ?>
                                                    </div>
                                                    <a href="<?php echo get_field('button')['link']; ?>" class="best__slide-btn btn-blue">
                                                        <span>
                                                            <?php echo get_field('button')['text']; ?>
                                                        </span>
                                                    </a>
                                                </div>
                                            </div>

                                        </div>
                                    <?php
                                        // Додатковий код або HTML для відображення додаткової інформації про казино
                                    endwhile;
                                    wp_reset_postdata(); // Скидаємо запит, щоб не конфліктував з іншими запитами на сторінці
                                endif;
                                ?>
                            </div>
                        </div>
                        <div class="best__nav">
                            <div class="best__prev">
                                <svg width="40" height="33" viewBox="0 0 40 33" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M39.9469 16.5C39.9469 16.7303 39.8554 16.9512 39.6925 17.1141C39.5297 17.2769 39.3088 17.3684 39.0785 17.3684H2.96626L17.1146 31.5167C17.2775 31.6797 17.3691 31.9007 17.3691 32.1311C17.3691 32.3616 17.2775 32.5826 17.1146 32.7455C16.9516 32.9085 16.7306 33 16.5002 33C16.2698 33 16.0488 32.9085 15.8858 32.7455L0.254689 17.1144C0.173948 17.0337 0.109897 16.938 0.0661954 16.8326C0.022494 16.7271 0 16.6141 0 16.5C0 16.3859 0.022494 16.2729 0.0661954 16.1675C0.109897 16.062 0.173948 15.9663 0.254689 15.8856L15.8858 0.254488C15.9665 0.173805 16.0623 0.109804 16.1677 0.0661391C16.2731 0.0224738 16.3861 0 16.5002 0C16.6143 0 16.7273 0.0224738 16.8327 0.0661391C16.9381 0.109804 17.0339 0.173805 17.1146 0.254488C17.1953 0.335171 17.2593 0.430956 17.3029 0.536373C17.3466 0.64179 17.3691 0.754776 17.3691 0.868879C17.3691 0.982982 17.3466 1.09597 17.3029 1.20138C17.2593 1.3068 17.1953 1.40259 17.1146 1.48327L2.96626 15.6316H39.0785C39.3088 15.6316 39.5297 15.7231 39.6925 15.886C39.8554 16.0488 39.9469 16.2697 39.9469 16.5Z" fill="#00D1FF"/>
                                </svg>
                            </div>
                            <div class="best__pagination swiper-pagination"></div>
                            <div class="best__next">
                                <svg width="40" height="33" viewBox="0 0 40 33" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M0.0531044 16.5C0.0531044 16.7303 0.144596 16.9512 0.307453 17.1141C0.47031 17.2769 0.691189 17.3684 0.921501 17.3684H37.0337L22.8854 31.5167C22.7225 31.6797 22.6309 31.9007 22.6309 32.1311C22.6309 32.3616 22.7225 32.5826 22.8854 32.7455C23.0484 32.9085 23.2694 33 23.4998 33C23.7302 33 23.9512 32.9085 24.1142 32.7455L39.7453 17.1144C39.8261 17.0337 39.8901 16.938 39.9338 16.8326C39.9775 16.7271 40 16.6141 40 16.5C40 16.3859 39.9775 16.2729 39.9338 16.1675C39.8901 16.062 39.8261 15.9663 39.7453 15.8856L24.1142 0.254488C24.0335 0.173805 23.9377 0.109804 23.8323 0.0661391C23.7269 0.0224738 23.6139 0 23.4998 0C23.3857 0 23.2727 0.0224738 23.1673 0.0661391C23.0619 0.109804 22.9661 0.173805 22.8854 0.254488C22.8047 0.335171 22.7407 0.430956 22.6971 0.536373C22.6534 0.64179 22.6309 0.754776 22.6309 0.868879C22.6309 0.982982 22.6534 1.09597 22.6971 1.20138C22.7407 1.3068 22.8047 1.40259 22.8854 1.48327L37.0337 15.6316H0.921501C0.691189 15.6316 0.47031 15.7231 0.307453 15.886C0.144596 16.0488 0.0531044 16.2697 0.0531044 16.5Z" fill="#00D1FF"/>
                                </svg>
                            </div>
                        </div>
                    </div>
                    <?php
                }
            }
            ?>
        </div>
    </div>
</section>